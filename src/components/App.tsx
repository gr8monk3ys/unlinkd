...
export function App(): React.JSX.Element {
  ...
  function handleTabKeyDown(event: React.KeyboardEvent): void {
    ...
    if (nextIndex >= 0) {
      ...
      setTab(tabItems[nextIndex]!.id);
      document.getElementById(`tab-${tabItems[nextIndex]!.id}`)?.focus();
    }
  }
  ...
  if (!isUnlocked || !vault || !persona) {
    return (
      <main>
        <a href="#unlock-form" className="skip-link">Skip to content</a>
        <h1>unlinkd</h1>
  ...
  return (
    <main>
      ...
      {tab === 'dashboard' ? (
        ...
        <DashboardTab
          personaIdentifiersCount={personaIdentifiers.length}
          personaAccountsCount={personaAccounts.length}
          graphNodes={exposureGraph.nodes.length}
          graphEdges={exposureGraph.edges.length}
          connectorInstancesCount={connectorInstances.length}
          dueConnectors={due}
          connectorCatalog={connectorCatalog}
          auditCount={auditCount}
          auditError={auditError}
          onMarkRechecked={(id) => void handleMarkRechecked(id)}
          onRunLocalScan={() => void handleRunLocalScan()}
          onVerifyAudit={() => void handleVerifyAudit()}
          exposureNodes={exposureGraph.nodes}
          exposureEdges={exposureGraph.edges}
      ...
      {tab === 'personas' ? (
        ...
        <PersonasTab
          ...
          onSetActivePersona={(id) => void handleSetActivePersona(id)}
          onAddPersona={(name) => void handleAddPersona(name)}
        />
        </div>
      ) : null}
      ...
      {tab === 'accounts' ? (
        ...
        <AccountsTab
          ...
          accountsImportStatus={accountsImportStatus}
          onAddAccount={handleAddAccount}
          onImportAccounts={(file) => void handleImportAccounts(file)}
          onImportMailbox={(file) => void handleImportMailbox(file)}
        />
        </div>
      ) : null}
      ...
      {tab === 'connectors' ? (
        ...
        <ConnectorsTab
          ...
          connectorInstances={connectorInstances}
          onUpdateCatalog={() => void handleUpdateConnectorCatalog()}
          onImportCatalog={(file) => void handleImportConnectorCatalog(file)}
          onImportAgentResults={(file) => void handleImportAgentResults(file)}
          onAddConnector={(def) => void handleAddConnector(def)}
          onExportAgentJob={(id) => void handleExportAgentJob(id)}
          onTransition={(id, to) => void handleTransition(id, to)}
          onAddNoteEvidence={handleAddNoteEvidence}
          onDeleteEvidence={(id, evidenceId) => void handleDeleteEvidence(id, evidenceId)}
          onUploadEvidence={handleUploadEvidence}
          onDownloadEvidence={(meta) => void handleDownloadEvidence(meta)}
        />
        </div>
      ) : null}
      ...
      {tab === 'findings' ? (
        ...
        <FindingsTab findings={prioritizedFindings} />
        </div>
      ) : null}

      {tab === 'report' ? (
        <div role="tabpanel" id="tabpanel-report" aria-labelledby="tab-report">
        <ReportTab onExportReport={(redacted) => void handleExportReport(redacted)} />
        </div>
      ) : null}
      ...
      {tab === 'backup' ? (
        ...
        <BackupTab
          ...
          onWipeAllData={() => void handleWipeAllData()}
        />
        </div>
      ) : null}
      </Suspense>
